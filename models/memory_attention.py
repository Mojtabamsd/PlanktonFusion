...
class MemoryAttentionModule(nn.Module):
    def __init__(self, query_size, memory_size, attention_units):
        ...
        self.dense_layer = nn.Linear(query_size, attention_units)
        self.softmax = nn.Softmax(dim=-1)
        self.normalize = nn.LayerNorm(memory_size)

    def forward(self, query, memory_keys):
        query_transformed = F.relu(self.dense_layer(query))
...
class MA(nn.Module):
    def __init__(self, model_name, weights_path, visual_encoder_size, query_size, memory_size, attention_units,
        ...
        self.memory_attention = MemoryAttentionModule(query_size, memory_size, attention_units)
